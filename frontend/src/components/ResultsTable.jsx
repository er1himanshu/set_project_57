  ...
  if (!results || results.length === 0) {
    return (
      <div className="card p-16 text-center animate-fade-in">
        ...
        <p className="text-gray-700 font-bold text-xl mb-2">No results yet</p>
        <p className="text-gray-500 text-lg">Upload some images to see analysis results here</p>
      </div>
    );
  }
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-gradient-to-r from-primary-600 to-secondary-600 text-white">
              ...
              <th className="px-6 py-4 text-left text-sm font-bold uppercase tracking-wider">Filename</th>
              <th className="px-6 py-4 text-left text-sm font-bold uppercase tracking-wider">Resolution</th>
              <th className="px-6 py-4 text-left text-sm font-bold uppercase tracking-wider">Quality Score</th>
              <th className="px-6 py-4 text-left text-sm font-bold uppercase tracking-wider">Background</th>
              <th className="px-6 py-4 text-left text-sm font-bold uppercase tracking-wider">Image-Text Match</th>
              <th className="px-6 py-4 text-left text-sm font-bold uppercase tracking-wider">Status</th>
              <th className="px-6 py-4 text-left text-sm font-bold uppercase tracking-wider">Issues</th>
            </tr>
          </thead>
          <tbody>
            {results.map((r, index) => (
              <tr 
                key={r.id} 
                className={`border-b border-gray-100 hover:bg-primary-50 transition-all duration-200 ${
                  index % 2 === 0 ? 'bg-white' : 'bg-gray-50'
                }`}
              >
                <td className="px-6 py-5 text-sm text-gray-900 font-bold">#{r.id}</td>
                ...
                <td className="px-6 py-5 text-sm">
                  <div className="text-gray-800 font-bold">{r.width}×{r.height}</div>
                  <div className="text-xs text-gray-600 font-medium">{r.aspect_ratio?.toFixed(2)} ratio</div>
                </td>
                <td className="px-6 py-5 text-sm">
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-gray-600 font-medium w-16">Blur:</span>
                      <span className={`text-xs font-bold ${r.blur_score >= 100 ? 'text-success-600' : 'text-danger-600'}`}>
                        {r.blur_score?.toFixed(0)}
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-gray-600 font-medium w-16">Sharp:</span>
                ...
                <td className="px-6 py-5 text-sm">
                  <div className="flex items-center">
                    ...
                    <span className="text-xs text-gray-700 font-bold">{(r.background_score * 100).toFixed(0)}%</span>
                  </div>
                  {r.has_watermark && (
                ...
                <td className="px-6 py-5 text-sm">
                  {r.similarity_score !== null && r.similarity_score !== undefined ? (
                    ...
                    <span className="text-xs text-gray-500 italic">No description</span>
                  )}
                </td>
                  ...
                  {r.passed ? (
                    ...
                    <div className="text-danger-700">
                      ...
                      {r.improvement_suggestions && (
                        <details className="text-xs text-gray-700 mt-3">
